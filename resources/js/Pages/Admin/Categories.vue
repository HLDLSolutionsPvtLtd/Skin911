           ...
           <div style="height:calc(100vh - 80px);" class="m-2 mx-4 container-fluid bg-white rounded p-1">
               <div class="m-2 relative">
                   <div class="flex justify-between mb-2 border-b-2 pb-2">
                       <div class="">
                           <button @click="newcategory = !newcategory" class="p-2 bg-green-500 text-white rounded-md text-xs tracking-wider font-bold">NEW</button>
                       </div>
                       <div class="">
                   ...
                   <div v-show="newcategory" class="absolute bg-white w-full p-4 border shadow-lg">
                       ...
                       <div class="mt-4 flex gap-4">
                           ...
                           <div>
                               <button @click="newcategory = !newcategory" class="bg-red-500 font-bold text-white text-xs tracking-wider p-2 rounded-md">CLOSE</button>
                           </div>
                       </div>
                   </div>
                   <div v-show="updatecategory" class="absolute bg-white w-full p-4 border shadow-lg">
                       <div class="flex border-b-2 justify-between">
                           <div>
                       ...
                       <div class="flex mt-4 flex-col">
                           ...
                           <div class="mt-4">
                                ...
                                <div v-if="!editpreview" class="mt-2 border border-dashed border-gray-400">
                                    <div class="text-sm  p-12 tracking-wider text-center items-center text-blue-400">
                                        <input id="image" class="opacity-0 absolute -z-1" accept=".jpg, .png" type="file" ref="file" name="image" @change="newcategoryimage">
                                        <span>Drag and Drop / Click to Add image</span>
                                    </div>
                                </div>
                                <div v-else>
                       ...
                       <div class="mt-4 flex gap-4">
                           ...
                           <div>
                               <button @click="updatecategory = !updatecategory" class="bg-red-500 font-bold text-white text-xs tracking-wider p-2 rounded-md">CLOSE</button>
                           </div>
                       </div>
                   </div>
                   <table class="border border-gray-300 w-full text-ct">
                        ...
                        <tr v-for="category in categories" :key="category.id" class="border-b text-sm text-gray-500 tracking-wider border-gray-300">   
                            ...
                            <td class="p-2 flex border-r border-gray-300"><img v-if="category.img" class="h-14 w-14 flex mr-1" :src="'/storage/'+category.img" alt=""></td>
                            <td class="p-2">
                                <div class="flex gap-2 justify-center items-center">
                                    <button @click="updatecategory = !updatecategory, newform.name = category.name, newform.id = category.id" class="flex items-center justify-center font-bold text-sm text-red-600 p-2 bg-green-400 rounded-md shadow-lg">
                                    ...
                                    <button @click="deletecategory(category)" class="flex items-center justify-center font-bold text-sm text-red-600 p-2 bg-green-400 rounded-md shadow-lg">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" class="fill-current text-red-800" viewBox="0 0 24 24">
...
<script>
 ...
 export default{
    ...
    methods:
    {
        ...
        update(){
            this.newform.post('/admin/category/'+this.newform.id+'/update', {
                onSuccess:() =>
                {
                    alert("Success");
        ...
        deletecategory(category){
            if(confirm('this category will be permanently deleted'))
            {
                axios.delete('/admin/category/'+category.id+'/delete')
                .then(
